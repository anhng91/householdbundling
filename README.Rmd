...
The computation of $\exp(-1/2 (\log (\pmb{\theta} - \pmb{\bar{\theta}}) \sigma_{\theta|\pmb{\bar{\theta}}}^{-2} \log (\pmb{\theta} - \pmb{\bar{\theta}})')$ requires knowing $\sigma_{\theta}$. So instead of computing this function directly, I use a second-order Taylor approximation around $-1/2 (\log (\pmb{\theta} - \pmb{\bar{\theta}}) \sigma_{\theta}^{-2} \log (\pmb{\theta} - \pmb{\bar{\theta}})'$
\[
