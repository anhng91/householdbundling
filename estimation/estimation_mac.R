...
# sample_r_theta = sample(Vol_HH_list_index[which(!(is.na(mclapply(Vol_HH_list_index, function(x) ifelse(nrow(data_hh_list[[x]]) <= 4, x, NA)))))], 5000)
...
compute_moment_theta = function(x_transform) { 
    ...
    result[[1]] = mclapply(theta_estimate_output, function(x) x[[1]], mc.cores=numcores) %>% unlist; 
    ...
    for (varname in var_list) {
      if (length(na_index) == 0) {
        derivative[[varname]] = Reduce('+', mclapply(theta_estimate_output, function(x) x[[2]][[varname]], mc.cores=numcores))
      } else {
        derivative[[varname]] = Reduce('+', mclapply(theta_estimate_output[-na_index], function(x) x[[2]][[varname]], mc.cores=numcores))
      }
...
mat_M = do.call('rbind', mclapply(data_hh_list[sample_r_theta], function(x) {
    return(cbind(x$M_expense, x$M_expense^2))
  }, mc.cores=numcores))

mat_Y = do.call('c', mclapply(data_hh_list[sample_r_theta], function(x) x$Income, mc.cores=numcores))
...
relevant_index = mclapply(1:length(sample_r_theta), function(index) {
  output = list();
...
relevant_index_na = mclapply(1:length(relevant_index), function(id) ifelse(length(relevant_index[[id]][[2]]) > 0, id, NA), mc.cores=numcores) %>% unlist()
...
full_insurance_indicator_ind_level_nona = do.call('c', mclapply(data_hh_list[sample_r_theta[which(!(is.na(relevant_index_na)))]], function(x) {
    return(rep(x$HHsize_s[1] == x$N_vol[1], x$HHsize[1]))
  }, mc.cores=numcores))

mat_M = do.call('rbind', mclapply(data_hh_list[sample_r_theta[which(!(is.na(relevant_index_na)))]], function(x) {
    return(cbind(x$M_expense, x$M_expense^2))
  }, mc.cores=numcores))

X_hh_all = do.call('rbind',mclapply(moment_eligible_hh_output_lower, function(output_hh) output_hh$X_hh, mc.cores=numcores))

vec_Y_nona = do.call('c', mclapply(data_hh_list[sample_r_theta[which(!(is.na(relevant_index_na)))]], function(x) {
    return(x$Income[1])
  }, mc.cores=numcores))
...
estimate_r_thetabar = optimize(function(x) {
  ...
  if (Sys.info()[['sysname']] == 'Windows') {
    ...
    moment_eligible_hh_output = mclapply(relevant_index_nona, function(mini_data_index) tryCatch(household_draw_theta_kappa_Rdraw(mini_data_index[[1]], x_transform[[1]], n_halton_at_r, 10, sick_parameters, xi_parameters, u_lowerbar = -1, relevant_index = mini_data_index[[2]]), error=function(e) e), mc.cores=numcores)
  }
  ...
  root_r_none_inf = do.call('c', mclapply(relevant_index, function(index) c(1:n_halton_at_r) %in% index[[2]], mc.cores=numcores))
  fx_r = function(x_r, derivative=FALSE) {
    ...
    if (derivative) {
      ...
      d_output_mean = do.call('c', mclapply(1:ncol(d_prob_mean), function(id_col) mean(matrix(((-d_prob_mean[,id_col])*(1 - prob_0) + d_prob_0_mean[,id_col]*(1 - prob))/(1 - prob_0)^2, nrow=n_halton_at_r) %>% colMeans * (full_insurance_indicator_nona - (matrix((1 - prob)/(1 - prob_0),nrow=n_halton_at_r) %>% colMeans))), mc.cores=numcores))
    ...
    print('actual insurance = '); print(summary(c(full_insurance_indicator_nona)))
